...
export function LoginForm({ compact = false }: { compact?: boolean }) {
  ...
  const blocked = Boolean(
    auth.session && !auth.loading && !auth.isStudent,
  );
  ...
  return (
    <div>
      <p className="text-body-md text-on-surface-variant">
        {compact
          ? "Students sign in with their school Google account."
          : "Students sign in with their school Google account. The handbook answers are yours after that."}
      </p>
      ...
      {blocked ? (
        <p className="mt-4 rounded-2xl bg-error-container px-3 py-2 text-body-md text-on-error-container">
          {auth.role === "staff"
            ? "This is for students only. Use a different account."
            : "This Google account isn’t on the school list. Use your UWC Costa Rica email."}
        </p>
      ) : null}
